...
AssetPriorityManager::AssetPriorityManager()
{
	...
	shared_ptr<PriorityNode> NormalTexture = make_shared<PriorityNode>(EAssetType::NormalTexture);
	...
	PriorityNodes.emplace_back(NormalTexture);
	PriorityNodes.emplace_back(EXRTexture);
	PriorityNodes.emplace_back(DDSTexture);
	PriorityNodes.emplace_back(Animation);

	StaticMesh->AddPrerequisite(NormalTexture);

	SkeletalMesh->AddPrerequisite(Bone);
	SkeletalMesh->AddPrerequisite(NormalTexture);

	Animation->AddPrerequisite(Bone);

	Material->AddPrerequisite(NormalTexture);
	...
	Map->AddPrerequisite(EXRTexture);
	Map->AddPrerequisite(DDSTexture);
}
