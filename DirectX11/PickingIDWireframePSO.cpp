...
PickingIDWireframePSO::PickingIDWireframePSO(
...
void PickingIDWireframePSO::Render()
{
	for (ABoundingObject* RenderBoundingObject : ABoundingObjects)
	{
		...
		if (RenderBoundingObject->GetIsRenderable() && DebugObject)
		{
			...
			ID3D11Buffer* PSConstBuffers[] = { RenderBoundingObject->GetPickingIDBufferCached() };
...
void PickingIDWireframePSO::ResetRendering()
{
