...
void ComponentUpdater::Visit(StaticMeshComponent* staticMeshComponent)
{
	try
	{
		...
		staticMeshTable.update()
			...
			.where("static_component_id = :static_component_id")
			.bind("static_component_id", componentID).execute();
	}
...
void ComponentUpdater::Visit(SkeletalMeshComponent* skeletalMeshComponent)
{
	try
	{
		...
		skeletalMeshTable.update()
			...
			.where("skeletal_component_id = :skeletal_component_id")
			.bind("skeletal_component_id", componentID).execute();
	}
	catch (const std::exception& ex)
	{
		OnErrorOccurs(ex.what());
