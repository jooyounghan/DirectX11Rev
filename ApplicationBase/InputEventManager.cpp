...
MouseEvent& InputEventManager::GetMouseEvent(EMouseAction MouseActionEnum)
{
...
void InputEventManager::ManageInputFromWindow(
	const UINT& width, const UINT& height, 
	UINT msg, WPARAM wParam, LPARAM lParam
)
{
	const size_t KeyValue = static_cast<size_t>(GetKeyInputFromWinMsg(wParam));
	const float NdcX = (GET_X_LPARAM(lParam) - (width / 2.f)) / (width / 2.f);
	const float NdcY = (GET_Y_LPARAM(lParam) - (height / 2.f)) / (height / 2.f);
